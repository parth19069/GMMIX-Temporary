...
parser.add_argument('--classifier_lr', type=float,default=1e-5,help="classifier initial learning rate")
...
def do_train(trainloader,clip_model,optimizer,epoch,args,classification_model=None,classification_optimizer=None,logits_scale2=None):
    ...
    for batch_idx,sample in enumerate(tqdm(trainloader)):
        ...
        # loss    +=  clip_loss(
        #         logits_per_image*(clip_model.module.logit_scale.exp()/args.divt),
        ...
        if epoch > -1 :
            ...
            #     logits_per_image *= clip_model.module.logit_scale.exp()
            ...
            #     mix_logits = mix_logits*clip_model.module.logit_scale.exp()/args.divt
            ...
            #     loss_mix += 0.1*calc_mix_loss(logits_per_image2*clip_model.module.logit_scale2.exp(), 1)
            ...
            #     logits_per_image2   = logits_per_image2*clip_model.module.logit_scale2.exp()/args.divt
            #     logits_per_text2    = logits_per_text2*clip_model.module.logit_scale2.exp()/args.divt
            ...
            if args.perform_classification:
                ...
                input_representation = metadata_features.to(device)
...
def do_valid(validloader,clip_model,optimizer,args,run_calib=True,wandb_prefix="",epoch=0):
    ...
    with torch.no_grad():
        ...
        scale   =  clip_model.module.logit_scale.exp() 
        scale2  =  clip_model.module.logit_scale2.exp() 
...
def do_classifier_test(validloader,clip_model,optimizer,args,classifier_model, run_calib=True,wandb_prefix="",epoch=0):
    ...
    with torch.no_grad():
        ...
        #     text_features = torch.mean(text_features, dim=1)
            ...
            input_representation = torch.Tensor(metadata).to(device)
...
nft_classification_model = ClassificationHead2(embedding_dim=512, metadata_dim=1, n_classes=2)
...
if args.noclip and args.albef :
    ...
    clip_model.module.logit_scale.data = clip_model.temp.data.log()*-1
...
if args.dataset == "nft":
    trainset = NFTDataset(folder_path = '/scratch/puneetm2/data/nft_data_updated/final_data', image_folder_path='/scratch/puneetm2/data/nft_data',image_lookback=args.img_lookback, tweet_lookback=args.text_lookback, transform=preprocess).filter_df("train")
    testset = NFTDataset(folder_path = '/scratch/puneetm2/data/nft_data_updated/final_data', image_folder_path='/scratch/puneetm2/data/nft_data',image_lookback=args.img_lookback, tweet_lookback=args.text_lookback, transform=preprocess).filter_df("test")
    # validset = 
